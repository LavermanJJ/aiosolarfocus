...
class _Prober:
    ...
    async def _detect_system(self, heat_pump_is_modern: bool, heating_circuit_states: list[int | None]) -> tuple[bool, bool, Systems]:
        ...
        if has_heat_pump or not has_biomass_boiler:
            ...
            # `CHIMNEY_SWEEP_HOLDING` says how far that goes - see its own
            # comment for why a mapped reading, now seen on a real ecotop
            # (#237, lein1013), settles nothing, and why the guess still breaks
            # towards the pellet elegance regardless.
